...
static void on_confirm_button_clicked(GtkWidget *button, gpointer data) {
    ...
    switch ((int)data){
        case ADD_FRIEND:
            result = add_friend(strtol(idText,NULL,10));
            switch (result){
                case FAILURE:
            ...
            break;
        case ADD_GROUP:
            result = join_group(strtol(idText,NULL,10));
            switch (result){
                case FAILURE:
                    show_error(NULL,NULL,"Sorry, Error!");
                    break;
...
void add_friend_window() {
    ...
    gtk_window_set_title(GTK_WINDOW(createWindow), "Add friend");
    ...
    usernamelabel = gtk_label_new("Friend Id:");
    ...
    g_signal_connect(G_OBJECT(confirmButton), "clicked", G_CALLBACK(on_confirm_button_clicked), ADD_FRIEND);
...
void join_group_window() {
    ...
    confirmButton = gtk_button_new_with_label("Confirm");
    g_signal_connect(G_OBJECT(confirmButton), "clicked", G_CALLBACK(on_confirm_button_clicked), ADD_GROUP);
    g_signal_connect_swapped(G_OBJECT(confirmButton), "clicked", G_CALLBACK(delete_event), NULL);
